...
void gfxPlatform::InitHardwareVideoConfig() {
...
#ifdef XP_WIN
  userPrefDec =
      userPrefDec && Preferences::GetBool("media.wmf.dxva.enabled", true);
#endif
  if (!userPrefDec) {
    featureDec.UserDisable("User disabled via pref",
  ...
  bool userPrefEnc =
      StaticPrefs::media_hardware_video_encoding_enabled_AtStartup();
#ifdef XP_WIN
  userPrefEnc =
      userPrefEnc && Preferences::GetBool("media.wmf.dxva.enabled", true);
#endif
  if (!userPrefEnc) {
    featureEnc.UserDisable("User disabled via pref",
                           "FEATURE_HARDWARE_VIDEO_ENCODING_PREF_DISABLED"_ns);
  } else if (StaticPrefs::
