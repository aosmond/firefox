...
static MOZ_ALWAYS_INLINE void ProcessChunk_SIMD(const uint8_t*& aSrc,
  ...
  if constexpr (Unroll) {
    // Process two vectors per iteration. This gives the compiler two
    // independent dependency chains to fill the execution ports and halves the
    // loop/branch overhead per pixel, matching what it does when
    // auto-vectorizing the scalar fallback.
...
// Swap the R and B bytes within each pixel (exchange byte 0 and byte 2 of
// each 32-bit lane). The portable form is a byte shuffle, which lowers to a
// single (v)pshufb on SSSE3/AVX2 and tbl on NEON. SSE2 has no byte shuffle,
// so it specializes this to a 32-bit rotate + masks (SwizzleSSE2.h): xsimd's
// portable byte shuffle would otherwise emulate the permute with
// unpack/pshuflw/pshufhw/packuswb -- 7 ops all bound to the single shuffle
// port.
template <class Arch>
...
// Force each pixel's alpha (the high byte of its 32-bit lane) to 255, making
// the pixel opaque.
template <class Arch>
...
static MOZ_ALWAYS_INLINE xsimd::batch<uint8_t, Arch> PremultiplyVector_SIMD(
    const xsimd::batch<uint8_t, Arch>& aSrc) {
  // Split the two channels packed in each 16-bit lane in place, so each
  // channel sits alone in a 16-bit lane with the high byte free (no widen
  // needed).
  ...
  // For a non-opaque destination, force the alpha channel's input to 255 so
  // the multiply yields 255*A/255 = A, preserving the original alpha with no
  // later select. (ga holds G in even lanes and A in odd lanes; as uint32 the
  // A byte sits at bits [23:16] of each pixel.)
  if constexpr (!aOpaqueAlpha) {
    ga =
...
static MOZ_ALWAYS_INLINE xsimd::batch<uint8_t, Arch> UnpremultiplyReverse_SIMD(
    ...
    const xsimd::batch<uint16_t, Arch>& aRb,
    const xsimd::batch<uint16_t, Arch>& aGa) {
  // Split each reciprocal into low and high 16-bit halves, each duplicated
  // into both 16-bit words of its lane to line up with the R/B and G/A word
  // layout (Qn Qn per pixel): qLo = Q & 0xFFFF, qHi = Q >> 16.
...
// Unpremultiply a vector of 4 pixels using splayed math and a reciprocal
// table that avoids doing any actual division.
...
static MOZ_ALWAYS_INLINE xsimd::batch<uint8_t, Arch> UnpremultiplyVector_SIMD(
  ...
  // (a 16-bit rotate of each 32-bit lane), avoiding a byte shuffle which has
  // no vectorized form on 32-bit NEON.
  if constexpr (aSwapRB) {
...
static MOZ_ALWAYS_INLINE xsimd::batch<uint8_t, Arch> UnpackBatchRGB24_SIMD(
  ...
  constexpr auto expandMask =
      xsimd::make_batch_constant<uint8_t, UnpackRowRGB24ExpandMask<aSwapRB>,
                                 Arch>();
  // Expand the packed RGB pixels to RGBX and force each alpha byte to 255.
...
void MOZ_ALWAYS_INLINE UnpackRowRGB24_SIMD(const uint8_t* aSrc, uint8_t* aDst,
  ...
  // Each pass reads batchBytes worth of data, which maps to batchPackedSize
