...
static MOZ_ALWAYS_INLINE xsimd::batch<uint8_t, Arch> LoadRemainder_SIMD(
    const uint8_t* aSrc, size_t aLength) {
  // problem is that if we have >= 4, px2 contains the first pixel data, px the
  // second data if we have < 4, px2 contains the second pixel data (dead), px
  // the first data
  if (aLength < 4) {
...
static MOZ_ALWAYS_INLINE void StoreRemainder_SIMD(
  ...
  if constexpr (std::is_same_v<Arch, xsimd::avx2>) {
    if (aLength >= 4) {
      ...
      px = _mm256_extractf128_si256(aSrc, 0);
    }
  } else {
    px = aSrc;
  }
