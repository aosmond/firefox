                           ...
                           AVPacket *pkt, bool wait)
{
    ...
    while (offset < pkt->size || (need_draining && !s->draining)) {
        ...
        if (need_draining) {
            ...
            if (avctx->moz_ndk_crypto_info) {
                status = ff_AMediaCodec_queueSecureInputBuffer(codec, index, 0, avctx->moz_ndk_crypto_info, pts, flags);
            } else {
        ...
        if (avctx->moz_ndk_crypto_info) {
            status = ff_AMediaCodec_queueSecureInputBuffer(codec, index, 0, avctx->moz_ndk_crypto_info, pts, 0);
        } else {
