...
inline bool MatchingOperatingSystems(OperatingSystem aBlockedOS,
  ...
  if (aBlockedOS == OperatingSystem::Windows10or11 &&
      (aSystemOS == OperatingSystem::Windows10 ||
       aSystemOS == OperationSystem::Windows11)) {
    return true;
  }
#endif
...
inline bool MatchingRefreshRateStatus(RefreshRateStatus aBlockedStatus,
                                      RefreshRateStatus aSystemStatus) {
...
inline bool MatchingRefreshRates(int32_t aSystem, int32_t aBlocked,
                                 int32_t aBlockedMax,
...
int32_t GfxInfoBase::FindBlocklistedDeviceInList(
  ...
  for (; i < info.Length(); i++) {
    ...
    if (!MatchingRefreshRateStatus(info[i]->mRefreshRateStatus,
                                   refreshRateStatus)) {
      continue;
    }
