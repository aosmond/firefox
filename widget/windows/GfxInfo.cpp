...
nsresult GfxInfo::Init() {
  ...
  if (spoofedWindowsVersion) {
    ...
    nsString ubrStr;
    uint32_t ubr = 0;
    constexpr auto kUbrKeyName =
        u"Software\\Microsoft\\Windows NT\\CurrentVersion\\UBR"_ns;
    if (WinRegistry::GetString(HKEY_LOCAL_MACHINE, kUbrKeyName, u""_ns,
                               ubrStr)) {
      nsresult convertRv;
      ubr = ubrStr.ToUnsignedInteger(&convertRv);
    }
